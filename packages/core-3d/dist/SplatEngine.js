...
export const SPATIAL_METRIC_EVENT = 'spatial-metric-logged';
export const SPATIAL_MODEL_SELECTED_EVENT = 'spatial-model-selected';
export const SPATIAL_MIDPOINT_EVENT = 'spatial-midpoint-moved';
/* ═══════════════════════════════════════════════════════════════════════════
   TUNING CONSTANTS
 ══════════════════════════════════════════════════════════════════════════ */
/**
...
export class SplatEngine {
    ...
    _measureLineColor = new pc.Color(0.502, 0, 0.502, 1);
    /* ── Injection subsystem ───────────────────────────────────────────────── */
    injectedModels = new Map();
    activeSelection = null;
    activeSelectionHandle = null;
    lastGroundHit = new pc.Vec3(0, 0, 0);
    ...
    handleMouseMove(e) {
        ...
        this.lastMouseX = e.clientX;
        this.lastMouseY = e.clientY;
        if (this.interactionMode !== 'navigate' || !this.cameraEntity)
            return;
    ...
    /* ═══════════════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════════════ */
    handleKeyDown(e) {
        if (this.interactionMode !== 'inject' || !this.activeSelection)
            return;
        ...
        switch (e.key) {
            case 'ArrowLeft':
                this.activeSelection.setPosition(pos.x - step, pos.y, pos.z);
                break;
            case 'ArrowRight':
                this.activeSelection.setPosition(pos.x + step, pos.y, pos.z);
                break;
            case 'ArrowUp':
                this.activeSelection.setPosition(pos.x, pos.y, pos.z - step);
                break;
            case 'ArrowDown':
                this.activeSelection.setPosition(pos.x, pos.y, pos.z + step);
                break;
     ...
     */
    dispatchRaycast(clientX, clientY) {
        if (this.interactionMode === 'inject') {
            ...
            if (coord)
                this.handleInjectGroundCapture(coord);
            return;
        }
     ...
     */
    intersectGroundPlane(clientX, clientY) {
        if (!this.app || !this.cameraEntity?.camera)
            return null;
        ...
        if (Math.abs(dir.y) < EPSILON)
            return null; // ray nearly parallel to plane
    ...
    performProxyMeshRaycast(clientX, clientY) {
        if (!this.cameraEntity?.camera || !this.proxyMeshEntity)
            return null;
    ...
    pickInjectedEntity(clientX, clientY) {
        if (!this.cameraEntity?.camera || this.injectedModels.size === 0)
            return null;
        const { ray, origin } = this.buildCameraRay(clientX, clientY);
    ...
    buildCameraRay(clientX, clientY) {
        const camera = this.cameraEntity.camera;
        const rect = this.canvas.getBoundingClientRect();
        const scaleX = this.canvas.width / rect.width;
        const scaleY = this.canvas.height / rect.height;
    ...
    closestAABBHit(ray, aabbs, from) {
        ...
        for (const aabb of aabbs) {
            if (aabb.intersectsRay(ray, candidate)) {
                ...
                if (d < closestDistSq) {
                    closestDistSq = d;
                    hitPoint = candidate.clone();
                }
            }
        }
        return hitPoint;
    }
    collectEntityAABBs(entity) {
        const boxes = [];
        const walk = (e) => {
     ...
     */
    handleMeasurementCapture(coord) {
        if (!this.anchorA) {
        ...
        window.dispatchEvent(new CustomEvent(SPATIAL_METRIC_EVENT, { detail: measurement }));
        this.coordinateCallback?.(coord);
        /*
    ...
    handleInjectGroundCapture(coord) {
    ...
    tickMeasureLine() {
        ...
        /* ── C. Midpoint ──────────────────────────────────────────────────── */
        const mid = new pc.Vec3((vecA.x + vecB.x) * 0.5, (vecA.y + vecB.y) * 0.5, (vecA.z + vecB.z) * 0.5);
        /* ── D. Project midpoint to CSS pixel coordinates ─────────────────── */
        const screen = this.worldToScreenCSS(mid);
        if (!screen)
            return;
        /* ── E. Badge position broadcast ─────────────────────────────────── */
        window.dispatchEvent(new CustomEvent(SPATIAL_MIDPOINT_EVENT, {
            detail: {
                screenX: screen.x,
                screenY: screen.y,
                worldX: mid.x,
    ...
    drawMeasureLine(start, end) {
        ...
        if (typeof app['renderLine'] === 'function') {
            app['renderLine'](start, end, this._measureLineColor);
        }
        else if (typeof app['drawLine'] === 'function') {
            app['drawLine'](start, end, this._measureLineColor);
        }
    }
    /**
    ...
    applySelection(entity) {
        ...
        window.dispatchEvent(new CustomEvent(SPATIAL_MODEL_SELECTED_EVENT, {
    ...
    applyDeselection() {
        ...
        window.dispatchEvent(new CustomEvent(SPATIAL_MODEL_SELECTED_EVENT, { detail: null }));
        console.log('[INJECT] Deselected.');
    }
    /* ═══════════════════════════════════════════════════════════════════════
       Ground-Snap Positioning
     ══════════════════════════════════════════════════════════════════════ */
    snapEntityToGround(entity, x, z) {
    ...
    /* ═══════════════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════════════ */
    async inject3DModel(modelUrl) {
        if (!this.app)
            throw new Error('[SplatEngine] Engine not initialized.');
        ...
        return new Promise((resolve, reject) => {
            if (!this.app)
                return reject(new Error('[SplatEngine] Engine not initialized.'));
            const app = this.app;
            ...
            asset.on('load', (loadedAsset) => {
                ...
                const pos = entity.getPosition();
                console.log(`[INJECT] Ready: ${handle} @ ` +
                    `(${pos.x.toFixed(3)}, ${pos.y.toFixed(3)}, ${pos.z.toFixed(3)})`);
                resolve(handle);
            });
            asset.on('error', (err) => reject(new Error(`[INJECT] Load fault [${modelUrl}]: ${err}`)));
            app.assets.add(asset);
            app.assets.load(asset);
        });
    }
    selectInjectedModel(handle) {
        const entity = this.injectedModels.get(handle);
        if (!entity) {
            console.warn(`[INJECT] Handle not found: ${handle}`);
            return;
        }
        this.applySelection(entity);
    }
    deselectModel() { this.applyDeselection(); }
    translateActiveSelection(dx, dz) {
        if (!this.activeSelection)
            return;
        const pos = this.activeSelection.getPosition();
        this.activeSelection.setPosition(pos.x + dx, pos.y, pos.z + dz);
    }
    removeInjectedModel(handle) {
        const entity = this.injectedModels.get(handle);
        if (!entity)
            return;
        if (this.activeSelection === entity)
            this.applyDeselection();
        entity.destroy();
        this.injectedModels.delete(handle);
        console.log(`[INJECT] Removed: ${handle}`);
    }
    getActiveSelectionHandle() { return this.activeSelectionHandle; }
    getLastGroundHit() {
     ...
     */
    async loadSavedLayoutBuild(assets) {
        if (!this.app) {
        ...
        const results = await Promise.allSettled(assets.map((record, idx) => this.loadSavedAssetRecord(record, idx)));
     ...
     */
    loadSavedAssetRecord(record, slotIndex) {
        return new Promise((resolve) => {
            ...
            if (typeof url !== 'string' || url.trim() === '') {
                console.warn(`[SplatEngine] Layout record[${slotIndex}] is missing a valid modelUrl — skipped.`);
                resolve();
                return;
            }
            ...
            asset.on('load', (loadedAsset) => {
                ...
                this.injectedModels.set(handle, entity);
                console.log(`[SplatEngine] Restored[${slotIndex}]: ${record.name ?? url} ` +
            ...
            asset.on('error', (err) => {
                ...
                console.warn(`[SplatEngine] Layout record[${slotIndex}] load fault [${url}]: ${err} — skipped.`);
                resolve();
            });
            app.assets.add(asset);
            app.assets.load(asset);
    ...
    /* ═══════════════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════════════ */
    async loadSplatLocation(splatUrl, collisionMeshUrl) {
        if (!this.app)
            throw new Error('[SplatEngine] Engine not initialized.');
    ...
    flushSceneAssets() {
        ...
        if (this.proxyMeshEntity) {
            this.proxyMeshEntity.destroy();
            this.proxyMeshEntity = null;
        }
    }
    loadGaussianSplat(url) {
        return new Promise((resolve, reject) => {
            ...
            asset.on('error', (err) => reject(new Error(`Splat load fault: ${err}`)));
            app.assets.add(asset);
            app.assets.load(asset);
        });
    }
    async loadCollisionMeshProxy(url) {
        if (!this.app)
            return;
        ...
        try {
            ...
            if (!probe.ok) {
                console.log('[SplatEngine] Collision mesh proxy absent; ' +
        ...
        return new Promise((resolve) => {
            ...
            asset.on('load', (loadedAsset) => {
                const proxy = loadedAsset.resource.instantiateRenderEntity();
            ...
            asset.on('error', (err) => {
                ...
                console.log(`[SplatEngine] Collision mesh proxy absent (${err}); ` +
                    'fallback to infinite ground grid plane activated.');
...
//# sourceMappingURL=SplatEngine.js.map